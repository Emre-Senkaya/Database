...
def prepp_kjoretoy(inn_fil: Union[str, pathlib.Path]) -> pl.DataFrame:
    ...
    # !!!! ERSTATT MED DIN KODE:
    drivstoffkoder = pl.read_csv(STATIC_DATA / "drivstoffkoder.csv", separator=";").with_columns(
        pl.col("kode").cast(pl.String)
    )

    df = df.join(drivstoffkoder, left_on="tekn_drivstoff", right_on="kode", how="left")

    df = df.group_by(pl.col("*").exclude("elbil")).agg(
        (pl.col("tekn_drivstoff") == "5").any().cast(pl.String).alias("elbil")
    ).with_columns(
    pl.col("elbil")
)
